...
# -*- coding: utf-8 -*-
"""
Génération automatique d'un article de blog — La Casa Del Cookie.
...
Quand la réserve de sujets non traités passe sous TOPIC_RESERVE_MIN, le script
regénère un lot de sujets et les ajoute à BLOG_WORKFLOW.md avant de rédiger :
le blog ne tombe plus à court de sujets et ne sort plus en code 78.
...
Options :
  ...
  --topics-only   réapprovisionne la réserve de sujets, sans rédiger d'article
"""
...
#  · TOPIC_RESERVE_MIN : sous ce nombre de sujets NON TRAITÉS, on regénère.
#    Huit sujets = deux mois de publication hebdomadaire, de quoi absorber
#    plusieurs échecs de réapprovisionnement d'affilée sans jamais tomber à sec.
#  · TOPIC_BATCH      : taille du lot demandé (avant déduplication).
#  · TOPIC_MAX_CALLS  : appels OpenAI maximum pour constituer un lot.
#  · TOPICS_MODEL     : modèle dédié aux sujets, indépendant de cfg["model"]
#    qui reste celui de la rédaction des articles.
TOPIC_RESERVE_MIN = 8
TOPIC_BATCH = 40
TOPIC_MAX_CALLS = 2
TOPICS_MODEL = "gpt-4o"
...
def parse_topics(workflow: str) -> list[dict]:
    ...
    Adapté à ce dépôt : les sujets y sont présentés sous forme de TABLEAU
    markdown (« | 1 | Sujet | Angle | »). Le format en liste numérotée
    (« 1. **Sujet** — angle ») reste accepté en repli, pour ne pas dépendre
    d'une réécriture de BLOG_WORKFLOW.md.
    """
    ...
    for line in block.splitlines():
        ...
        if len(cells) < 2 or not cells[0].isdigit():
            continue                      # en-tête, séparateur, ligne de texte
        title = re.sub(r"[`*✅]", "", cells[1]).strip()
        if not title:
            continue
        brief = re.sub(r"[`*✅]", "", cells[2]).strip() if len(cells) > 2 else ""
        published_m = re.search(r"`([a-z0-9\-]+)`", cells[1] + " " + (cells[2] if len(cells) > 2 else ""))
        topics.append({
            "num": int(cells[0]),
            "title": title,
            "brief": brief,
    ...
    # ── Repli : format en liste numérotée « 1. **Sujet** — angle »
    if not topics:
...
def parse_editorial_rules(workflow: str) -> str:
    ...
    Adapté à ce dépôt : la section est un sous-titre de niveau 3
    (« ### d. Règles éditoriales — interdictions »). Le niveau 2 reste accepté.
    """
    m = re.search(r"^(#{2,3})\s+[^\n]*Règles éditoriales[^\n]*$(.*?)(?=^#{1,3}\s)",
...
def topic_is_pending(topic: dict, done_nums: set[int], slugs: set[str]) -> bool:
    ...
    deux définitions séparées finiraient par diverger, et le réapprovisionnement
    se déclencherait sur un décompte qui ne correspond pas à ce que le script
    publie réellement.
    """
...
def clean_line(text: str) -> str:
    """Normalise une ligne de sujet renvoyée par le modèle.

    Retire puce, numérotation et balisage markdown, et neutralise le caractère
    « | » qui casserait la ligne du tableau de BLOG_WORKFLOW.md.
    """
    out = str(text or "").strip()
    out = re.sub(r"^\s*[-*•‣]\s*", "", out)          # puce
    out = re.sub(r"^\s*\d+\s*[.)\]]\s*", "", out)    # numérotation
    out = out.replace("|", "/")                      # ne doit pas casser le tableau
    out = re.sub(r"[`*_#\[\]]+", "", out)            # balisage markdown
    out = re.sub(r"\s+", " ", out)
    return out.strip(" -—–:;/\t")
...
def dedupe_topics(candidates: list[dict], known_slugs: set[str]) -> list[dict]:
    ...
    La clé de déduplication est le SLUG, jamais le titre : c'est le slug qui
    porte l'idempotence de tout le pipeline (nom du dossier dans /blog/,
    résolution de topic_is_pending, refus d'écraser un fichier existant). Deux
    titres différents qui produisent le même slug sont un doublon effectif, et
    le second ne serait jamais publiable.
    """
    ...
    for candidate in candidates:
        if not isinstance(candidate, dict):
            continue
        title = clean_line(candidate.get("title"))
        angle = clean_line(candidate.get("angle")) or "conseil local"
        ...
        if not slug or slug in seen:
            continue
        seen.add(slug)
        out.append({"title": title, "angle": angle, "slug": slug})
    return out


def build_topics_prompt(cfg: dict, existing_titles: list[str]) -> tuple[str, str]:
    """Prompt de génération de sujets, ancré sur sector / location / geo_keywords."""
    existing = "\n".join(f"- {t}" for t in existing_titles) or "- (aucun)"

    system = f"""Tu es consultant SEO local pour des artisans français.
Tu proposes des SUJETS d'articles de blog, pas des articles.

Tu réponds UNIQUEMENT par un objet JSON valide, sans bloc de code markdown :

{{
  "topics": [
    {{"title": "intitulé du sujet, 40 à 100 caractères",
      "angle": "type d'angle — « mot-clé visé »"}}
  ]
}}

RÈGLES
- Chaque sujet doit être CONCRET et ACTIONNABLE : une question que se pose
  vraiment un client, ou une décision qu'il doit prendre. Jamais un thème vague.
- Ancrage local obligatoire : une bonne moitié des sujets nomme une ville ou une
  zone de la liste fournie. Les autres restent utiles à un lecteur du secteur.
- Angle SEO/conseil : le champ « angle » donne le registre (conseil, guide,
  comparatif, événementiel, saisonnier, coulisses, service, conversion…) suivi
  du mot-clé visé entre guillemets français.
- Varie les registres et les moments de l'année. Ne propose pas quinze variantes
  du même sujet.
- N'invente aucun produit, aucun service, aucun prix, aucun chiffre : un sujet
  doit pouvoir être rédigé à partir des seuls faits déjà publiés sur le site.
- Ne reprends AUCUN des sujets déjà listés ci-dessous, ni une reformulation.

SUJETS DÉJÀ COUVERTS — à ne pas reproposer
{existing}
"""

    user = f"""Propose exactement {TOPIC_BATCH} nouveaux sujets d'articles.

Entreprise : {cfg['site_name']} — {cfg['sector']}.
Zone : {cfg['location']}.
Ton éditorial du blog : {cfg['tone']}.

Mots-clés géographiques et métier à exploiter :
...
def parse_topics_response(payload: dict) -> list[dict]:
    ...
    clé employée et que les entrées soient des objets ou de simples chaînes."""
    if not isinstance(payload, dict):
        return []
    raw = None
    for key in ("topics", "sujets", "items", "results", "list"):
        if isinstance(payload.get(key), list):
            raw = payload[key]
            break
    if raw is None:
        # Repli : première valeur qui soit une liste.
        raw = next((v for v in payload.values() if isinstance(v, list)), [])

    out: list[dict] = []
    for entry in raw:
        if isinstance(entry, str):
            out.append({"title": entry, "angle": ""})
        elif isinstance(entry, dict):
            title = entry.get("title") or entry.get("titre") or entry.get("sujet")
            angle = entry.get("angle") or entry.get("keyword") or entry.get("mot_cle") or ""
            if title:
                out.append({"title": title, "angle": angle})
    return out


def mock_topics(cfg: dict, count: int = TOPIC_BATCH) -> list[dict]:
    """Sujets de démonstration pour --mock : aucun appel API, mais des intitulés
    tous distincts au slug, afin d'exercer réellement la déduplication."""
    cities = [c for c in cfg["geo_keywords"] if c[:1].isupper()] or [cfg["location"]]
    registres = ["conseil", "guide", "comparatif", "événementiel", "saisonnier",
                 "coulisses", "service", "conversion"]
    out = []
    for i in range(count):
        city = cities[i % len(cities)]
        registre = registres[i % len(registres)]
        out.append({
            "title": f"Sujet de démonstration numéro {i + 1} à {city} : que retenir ?",
            "angle": f"{registre} — « démonstration {i + 1} »",
        })
    return out
...
def generate_topics(cfg: dict, existing_titles: list[str], known_slugs: set[str],
    ...
    """Constitue un lot de sujets inédits, dans la limite de TOPIC_MAX_CALLS appels."""
    if use_mock:
        log("Mode MOCK : sujets de démonstration, aucun appel API.")
        return dedupe_topics(mock_topics(cfg), known_slugs)
    ...
    for attempt in range(1, TOPIC_MAX_CALLS + 1):
        followup = None
        if collected:
            already = "\n".join(f"- {t['title']}" for t in collected)
            followup = [{
                "role": "user",
                "content": (f"Tu as déjà proposé ces {len(collected)} sujets :\n{already}\n"
                            f"Il en manque {TOPIC_BATCH - len(collected)}. Propose "
                            "uniquement des sujets SUPPLÉMENTAIRES, sans reprendre les "
                            "précédents. Réponds par le seul objet JSON."),
            }]
        log(f"Génération de sujets — appel {attempt}/{TOPIC_MAX_CALLS}.")
        try:
            payload = generate_content(cfg, system, user, followup=followup,
                                       model=TOPICS_MODEL)
        except (ValueError, json.JSONDecodeError) as exc:
            fail(f"Appel de sujets n°{attempt} inexploitable : {exc}")
            break

        fresh = dedupe_topics(parse_topics_response(payload), seen)
        seen.update(t["slug"] for t in fresh)
        collected.extend(fresh)
        log(f"  {len(fresh)} sujets inédits retenus "
            f"(total {len(collected)}/{TOPIC_BATCH}).")
...
def append_topics_to_workflow(workflow: str, new_topics: list[dict]) -> tuple[str, int]:
    ...
    Le format est relu sur place, jamais supposé : on repère la dernière ligne
    de données du tableau et on poursuit la numérotation. Le slug n'est PAS
    écrit — il est déduit du titre par slugify(), exactement comme pour les
    sujets déjà présents.
    """
    m = re.search(r"^##\s+\d+\.\s+.*sujets d'articles suggérés.*$",
                  workflow, flags=re.M | re.I)
    if not m:
        raise ValueError("Section des sujets suggérés introuvable dans BLOG_WORKFLOW.md")

    sec_start = m.end()
    nxt = re.search(r"^##\s", workflow[sec_start:], flags=re.M)
    sec_end = sec_start + (nxt.start() if nxt else len(workflow) - sec_start)
    block = workflow[sec_start:sec_end]

    rows = list(re.finditer(r"^\|\s*(\d+)\s*\|.*\|\s*$", block, flags=re.M))
    if not rows:
    ...
    lines = "\n".join(f"| {next_num + i} | {t['title']} | {t['angle']} |"
                      for i, t in enumerate(new_topics))
    new_block = block[:last_end] + "\n" + lines + block[last_end:]
    return workflow[:sec_start] + new_block + workflow[sec_end:], next_num


def git_commit_file(path: Path, message: str) -> bool:
    """Commit d'un SEUL fichier. Retourne False si rien n'a changé.

    Isoler ce commit permet au workflow de pousser les sujets avant de tenter la
    rédaction : si l'article échoue ensuite, le lot de sujets est déjà sauvé.
    """
...
def replenish_topics(cfg: dict, topics: list[dict], done_nums: set[int],
                     slugs: set[str], *, use_mock: bool = False,
                     dry_run: bool = False, do_commit: bool = True) -> tuple[str | None, int]:
    """Regénère la réserve si elle passe sous TOPIC_RESERVE_MIN.

    Retourne (contenu rechargé de BLOG_WORKFLOW.md ou None, nombre de sujets
    ajoutés). Le contenu est renvoyé pour que l'appelant reparse la liste sans
    relire le disque.
    """
    pending = pending_topics(topics, done_nums, slugs)
    log(f"Réserve de sujets non traités : {len(pending)} "
    ...
    log(f"Réserve basse — génération d'un lot de {TOPIC_BATCH} sujets.")
    existing_titles = [t["title"] for t in topics]
    known_slugs = {slugify(t["title"]) for t in topics} | set(slugs)

    fresh = generate_topics(cfg, existing_titles, known_slugs, use_mock=use_mock)
    if not fresh:
        fail("Aucun sujet inédit obtenu — la réserve existante reste en place.")
        return None, 0
    ...
    if dry_run:
        log(f"DRY-RUN : {len(fresh)} sujets seraient ajoutés à partir du "
            f"n°{first_num} — BLOG_WORKFLOW.md non modifié.")
        for t in fresh[:5]:
            log(f"    · {t['title']}")
        if len(fresh) > 5:
            log(f"    … et {len(fresh) - 5} autres.")
        return None, 0

    WORKFLOW_PATH.write_text(updated, encoding="utf-8")
    log(f"BLOG_WORKFLOW.md : {len(fresh)} sujets ajoutés "
    ...
    if do_commit:
        try:
            ...
            # Le fichier est écrit : le commit échoué n'annule pas le travail,
            # il sera repris par le `git add -A` de l'étape de publication.
            fail(f"Commit des sujets impossible ({exc}) — fichier écrit malgré tout.")
...
- Ancres de liens : les libellés des liens internes doivent être descriptifs et
  ...
  mot comme « ici », « blog », « cookies », « contact ».
...
def generate_content(cfg: dict, system: str, user: str,
    ...
    toucher à cfg["model"], qui reste celui de la rédaction des articles."""
    try:
    ...
    log(f"Appel OpenAI (modèle {use_model}, temperature {cfg['temperature']})…")
...
def mock_content(cfg: dict, topic: dict) -> dict:
    ...
    targets = cfg["internal_link_targets"]
    filler = ("Entre Soumoulou, Pau et Tarbes, la question ne se pose pas tout à fait "
              "de la même manière selon le moment de la journée et selon ce que vous "
              "prévoyez d'en faire. Les distances restent courtes sur ce secteur, ce qui "
              "change beaucoup de choses dans la façon d'organiser une commande, un "
              "goûter ou une réception. Les habitudes des uns et des autres varient, et "
              ...
              "qu'à une minorité des situations rencontrées sur le terrain.")
    sections = []
    for i in range(7):          # 7 sections : le mock dépasse la cible de 1200
        ...
        if i == 0:
            ...
            content.append({"type": "p",
                            "text": f"Découvrez [toute la gamme de cookies]({targets[0]}) "
                                    f"et [les modalités de commande]({targets[1]})."})
        if i == 1:
...
def validate_content(data: dict, cfg: dict) -> list[str]:
    ...
    # Garde-fou repris de BLOG_WORKFLOW.md : aucun prix chiffré dans le corps.
    if re.search(r"\d\s*(€|euros?)\b", plain(body), re.I):
        errors.append("garde-fou : prix chiffré détecté dans le corps")
...
def _balanced_div(html: str, start: int) -> str:
    """Retourne le <div> ouvert à `start`, fermeture comprise (divs imbriqués)."""
    depth = 0
    for m in re.finditer(r"<div\b[^>]*>|</div>", html[start:]):
        depth += -1 if m.group(0).startswith("</") else 1
        if depth == 0:
            return html[start:start + m.end()]
    raise ValueError("Gabarit : <div> non équilibré.")


def split_template(reference_html: str) -> dict:
    """Découpe le gabarit relu en morceaux réutilisables.

    Conventions propres à CE site, différentes de celles du dépôt de référence :
      · pas de commentaire <!-- Article --> avant les JSON-LD ;
      · les trois blocs JSON-LD sont AU MILIEU du <head>, suivis des favicons :
        on isole donc un `head_top` (avant le premier bloc) et un `head_tail`
        (des favicons à </head>) pour ne rien perdre ;
    ...
    Le gabarit HTML n'est jamais modifié : c'est ce parseur qui s'y adapte.
    """
    parts: dict[str, str] = {}

    head_end = reference_html.find("</head>")
    ld_start = reference_html.find('<script type="application/ld+json">')
    if head_end == -1 or ld_start == -1 or ld_start > head_end:
        raise ValueError("Gabarit : </head> ou bloc JSON-LD introuvable dans le <head>.")

    # Dernier </script> situé avant </head> : fin de la zone JSON-LD.
    ld_end = reference_html.rfind("</script>", 0, head_end)
    if ld_end == -1 or ld_end < ld_start:
        raise ValueError("Gabarit : fin des blocs JSON-LD introuvable.")
    ld_end += len("</script>")

    parts["head_top"] = reference_html[:ld_start]              # DOCTYPE → metas
    parts["head_tail"] = reference_html[ld_end:head_end]       # favicons → </head>
    ...
    parts["footer"] = reference_html[main_end:]                # </main> → </html>

    main_html = reference_html[main_start:main_end]

    # Fil d'Ariane : on reprend le bloc du gabarit, le 3e <li> sera remplacé.
    crumb = re.search(r'<nav class="breadcrumb".*?</nav>', main_html, re.S)
    if not crumb:
        raise ValueError("Gabarit : <nav class=\"breadcrumb\"> introuvable.")
    parts["breadcrumb"] = crumb.group()

    # Bloc CTA de fin d'article (contient des div imbriqués).
    cta_at = main_html.find('<div class="article-cta">')
    parts["cta"] = _balanced_div(main_html, cta_at) if cta_at != -1 else ""

    # En-tête de la section FAQ (balise ouvrante + titre) et gabarit d'une entrée.
    faq_at = main_html.find('<section class="faq"')
    if faq_at == -1:
        raise ValueError("Gabarit : <section class=\"faq\"> introuvable.")
    faq_block = main_html[faq_at:main_html.index("</section>", faq_at)]
    item_at = faq_block.find('<div class="faq-item">')
    if item_at == -1:
        raise ValueError("Gabarit : aucun .faq-item dans la section FAQ.")
    # Réindenté au niveau de .article-inner : le découpage part de la balise
    # ouvrante et perd donc l'indentation d'origine de la première ligne.
    parts["faq_head"] = "        " + faq_block[:item_at].strip()

    # Lien « Retour au blog » en pied d'article.
    back = re.search(r'<p[^>]*>\s*<a class="post-card-link" href="/blog/">.*?</p>',
                     main_html, re.S)
    parts["back"] = back.group() if back else ""

    return parts


def build_head(parts: dict, cfg: dict, data: dict, url: str, today: dict, img: str) -> str:
    ...
    Les motifs sont adaptés aux balises NON auto-fermantes de ce site.
    """
    head = parts["head_top"]
    ...
    head = swap(r'<meta name="keywords" content="[^"]*">',
                f'<meta name="keywords" content="{esc(", ".join(cfg["geo_keywords"][:10]))}">',
                head)
...
def build_jsonld(cfg: dict, data: dict, url: str, today: dict, img: str) -> str:
    ...
    article = {
        ...
        # Renvoie au nœud LocalBusiness déclaré sur la page d'accueil.
        "about": {"@id": f"{cfg['site_url']}/#localbusiness"},
        "isPartOf": {"@type": "Blog", "@id": f"{cfg['site_url']}/blog/",
...
def render_blocks(blocks: list[dict], pad: str = "          ") -> str:
    ...
    for block in blocks:
        ...
        if kind in ("ul", "ol"):
            ...
            lines = "\n".join(f"{pad}  <li>{inline(i)}</li>" for i in items)
            out.append(f"{pad}<{kind}>\n{lines}\n{pad}</{kind}>")
        elif kind == "h3":
            out.append(f"{pad}<h3>{inline(block['text'])}</h3>")
        elif kind == "strong":
            out.append(f"{pad}<p><strong>{inline(block['text'])}</strong></p>")
        else:
            out.append(f"{pad}<p>{inline(block['text'])}</p>")
    return "\n\n".join(out)


def build_main(parts: dict, cfg: dict, data: dict, today: dict, img_path: str) -> str:
    """Le <main> complet, aux conventions de ce site : fil d'Ariane repris du
    gabarit, article-wrap / container / article-inner, visuel de couverture,
    corps dans .article-body, FAQ en .faq-item, CTA et lien retour du gabarit."""
    reading = max(3, round(content_word_count(data) / 200))

    # Fil d'Ariane : le 3e <li> du gabarit reçoit le libellé de l'article.
    crumb = parts["breadcrumb"]
    lis = re.findall(r"<li>.*?</li>", crumb, re.S)
    if len(lis) == 3:
        crumb = crumb.replace(lis[2], f"<li>{inline(data['breadcrumb'])}</li>", 1)
    ...
    alt = esc(f"{plain(data['h1'])} — {cfg['site_name']}")
  ...
  <article class="article-wrap">
    <div class="container">
      <div class="article-inner">
        ...
        <h1 class="article-title">{inline(data['h1'])}</h1>
        ...
        <div class="article-cover">
          <img src="{img_path}" alt="{alt}" width="1200" height="800" loading="eager">
        </div>
...
def cover_for(cfg: dict, topic: dict) -> str:
    """Visuel de couverture : rotation déterministe sur la liste configurée,
    repli sur og_image. Toutes les images existent déjà dans /images/."""
    images = cfg.get("cover_images") or []
    if not images:
        return cfg["og_image"]
    return images[(topic["num"] - 1) % len(images)]
...
def assemble(reference_html: str, cfg: dict, topic: dict,
    ...
    img_path = cover_for(cfg, topic)
    img_abs = f"{cfg['site_url']}{img_path}"

    head = build_head(parts, cfg, data, url, today, img_abs)
    jsonld = build_jsonld(cfg, data, url, today, img_abs)
    header = parts["header"].replace("<body>", f"<body>\n{marker}", 1)

    return (head + jsonld + parts["head_tail"] + "</head>" + header
            + build_main(parts, cfg, data, today, img_path) + parts["footer"])
...
def validate_assembled(html: str, cfg: dict, topic: dict) -> list[str]:
    ...
    for tag in ("</main>", "</article>", '<div class="article-body">',
                '<link rel="stylesheet" href="/assets/blog.css">'):
        if tag not in html:
            errors.append(f"assemblage : {tag} absent")
    if html.count("<section") != html.count("</section>"):
        errors.append("assemblage : balises <section> déséquilibrées")
    return errors
...
def update_blog_index(cfg: dict, topic: dict, meta: dict, today: dict) -> str:
    """Ajoute la carte de l'article, aux conventions de ce blog :
    .post-card > .post-card-media + .post-card-body."""
    ...
    cover = cover_for(cfg, topic)

    card = f"""
      <article class="post-card">
        <a href="{url}" aria-label="Lire l'article : {esc(headline)}">
          <div class="post-card-media" style="background-image:url('{cover}');">
            <span class="post-card-tag">{esc(cfg['default_article_section'])}</span>
          </div>
        </a>
        <div class="post-card-body">
          <p class="post-card-date"><time datetime="{today['iso']}">{today['fr']}</time></p>
          <h2 class="post-card-title">
            <a href="{url}">{esc(headline)}</a>
          </h2>
          <p class="post-card-excerpt">{esc(teaser)}</p>
          <a class="post-card-link" href="{url}">Lire l'article →</a>
        </div>
      </article>
"""
    ...
    # Ce blog n'expose pas de tableau "blogPost" dans son JSON-LD : on n'en
    # fabrique pas, plutôt que de modifier la structure existante de l'index.
    return html
...
def update_llms(cfg: dict, topic: dict, meta: dict) -> str | None:
    """Section « ## Articles du blog » de llms.txt (nom propre à ce dépôt)."""
    if not LLMS.exists():
        return None
    ...
    m = re.search(r"^## Articles du blog\s*$(.*?)(?=^## |\Z)", text, flags=re.M | re.S)
    if not m:
...
def refresh_entries(cfg: dict, topic: dict, meta: dict) -> list[str]:
    ...
    if card:
        new_card = re.sub(r'<p class="post-card-excerpt">.*?</p>',
                          f'<p class="post-card-excerpt">{esc(teaser)}</p>',
                          card.group(), count=1, flags=re.S)
        new_card = re.sub(r'(<a href="/blog/' + re.escape(slug) + r'/">)[^<]*(</a>)',
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Génère un article de blog La Casa Del Cookie.")
    ...
    parser.add_argument("--topics-only", action="store_true",
                        help="réapprovisionne la réserve de sujets, sans rédiger d'article")
    ...
    try:
        ...
        if args.topics_only:
            ...
            log(f"Mode SUJETS SEULS : terminé — {restant} sujets en réserve.")
            return EXIT_OK
